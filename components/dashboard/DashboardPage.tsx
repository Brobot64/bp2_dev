...
const DashboardPage: React.FC = () => {
  ...
  const [stats, setStats] = useState<{ blvckcards: number; users: number; subscribers: number }>({
    blvckcards: 0,
    users: 0,
    subscribers: 0,
  });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      ...
      try {
        const statsResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/dashboard/stats`, {
          headers: {
            Authorization: `${token}`,
          },
        });
        ...
        const accessResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/track/stats`, {
          headers: {
            Authorization: `${token}`,
          },
        });
        ...
        const revenueResponse = await axios.get(`${process.env.NEXT_PUBLIC_API_BASE_URL}/track/revenue-stats`, {
          headers: {
            Authorization: `${token}`,
          },
        });
        ...
        <div style={{ marginTop: '40px' }}>
          ...
          <ResponsiveContainer width="100%" height={400}>
            <LineChart data={accessData}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="count" stroke="#f035ff" activeDot={{ r: 8 }} />
            </LineChart>
          </ResponsiveContainer>

          {isAdmin && (
            <>
              <h3 style={{ marginTop: '40px' }}>Revenue Statistics (Last 24 Hours to 360 Days)</h3>
              <ResponsiveContainer width="100%" height={400}>
