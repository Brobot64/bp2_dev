...
const ConclusionPage: React.FC<EditConclusionProps> = ({ slug }) => {
        ...
        const fetchConclusionData = async () => {
            try {
                ...
                const response = await axios.get(
                    `${process.env.NEXT_PUBLIC_API_BASE_URL}/dashboard/blvckbox/${slug}/conclusion`,
                    {
                        headers: {
                            Authorization: `${token}`,
                        },
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        try {
            ...
            console.error('Error saving conclusion section:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className={style.cardContainer}>
            <div className={`${style.card} ${style.w25}`}>
                <div className={style.header}>Add Conclusion Section</div>
